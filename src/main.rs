...
pub fn check_load_status(
    asset_server: Res<AssetServer>,
    mut primitive: ResMut<PrimitiveHandle>,
) {
    //match asset_server.load_state(node.0)
    if let Some(handle) = &primitive.0 {
        ...
        match asset_server.load_state(handle) {
            ...
            bevy::asset::LoadState::Failed(asset_load_error) => println!("failed to load {:#}. REASON: {:#}", path, asset_load_error),
        }
    }
}
